...
def test_create_get_food_comp(test_client, food_comp_test_obj):
    ...
    params = {
        "food_cd": food_comp_test_obj["food_cd"]
    }
...
def test_create_update_delete_food_comp(test_client, food_comp_test_obj, food_comp_test_obj_update):
    ...
    response = test_client.put(
        f"/api/v1/food_comp/{food_comp_test_obj['food_cd']}", json=food_comp_test_obj_update
    )
    assert response.status_code == 200
    
    time.sleep(1)
    response = test_client.delete(f"/api/v1/food_comp/{food_comp_test_obj['food_cd']}")
